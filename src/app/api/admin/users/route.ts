...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [rows] = await p.execute(
      "SELECT " + q(C.id) + " as " + q("id") + ", " + q(C.name) + " as " + q("username") + ", " + q(C.email) + " as " + q("email") + ", " + q(C.rank) + " as " + q("rank") + ", " + q(C.balance) + " as " + q("balance") + ", " + q(C.lastlogin) + " as " + q("lastSeen") + ", " + q(C.created) + " as " + q("created") + " FROM " + t
    );
